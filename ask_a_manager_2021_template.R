...
ask_a_manager_cleaned  <- ask_a_manager_2021 %>% 
...
# prelims
main.currency <- "USD"
url.list <- paste0("https://www.bloomberg.com/quote/", ask_a_manager_cleaned["currency","annual_salary"], main.currency, ":CUR")
mylist <- data.frame(ask_a_manager_cleaned["currency","annual_salary"], url.list, stringsAsFactors = F)

#Loading the rvest package
require('rvest')

#Reading the HTML codes from the website
myfct <- function(url){
  require(rvest)
  wp <- read_html(url)
  rate <- html_text(html_nodes(wp, ".price"))
  return(rate)
}
rate <- as.numeric(sapply(url.list, myfct))
currency <- ask_a_manager_cleaned["currency","annual_salary"]
rates <- data.frame(rate, currency)

# Converting to EUR
converted.dat <- t(t(dat)*rates$rate)
converted.dat <- data.frame(converted.dat)
names(converted.dat) <- paste0("Former ", names(converted.dat))
